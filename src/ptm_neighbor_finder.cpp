...
void PTMNeighborFinder::Query::findNeighbors(size_t particleIndex, std::optional<Quaternion> targetOrientation){
    ...
    std::array<int8_t, PTM_MAX_INPUT_POINTS> remapPermutation{};
    std::array<int, PTM_MAX_INPUT_POINTS> tmp{};
    std::iota(tmp.begin(), tmp.end(), 0);
    for(size_t i = 0; i < remapPermutation.size(); ++i){
        remapPermutation[i] = static_cast<int8_t>(tmp[i]);
    }
    ...
    if(_structureType != StructureType::OTHER && targetOrientation){
        ...
        _templateIndex = ptm_remap_template(ptmType, _templateIndex, qtarget, qptm, remapPermutation.data());
    ...
    for(int i = 0; i < _list.size(); i++){
        Neighbor& neighbor = _list[i];
        int index = remapPermutation[i + 1];
        neighbor.index = _env.atom_indices[index];
        const double *p = _env.points[index];
        neighbor.delta = Vector3(p[0], p[1], p[2]);
        neighbor.distanceSq = neighbor.delta.squaredLength();

        if(_structureType == StructureType::OTHER){
            neighbor.idealVector = Vector3(0, 0, 0);
        }else{
            const double* q = ptmTemplate[i + 1];
            neighbor.idealVector = Vector3(q[0], q[1], q[2]);
        }

        if(_structureType != StructureType::OTHER){
            const auto structureType = readStructure(*_finder._structuresArray, neighbor.index);
            const auto orientation = readOrientation(*_finder._orientationsArray, neighbor.index);
            neighbor.disorientation = PTM::calculateDisorientation(_structureType, structureType, _orientation, orientation);
        }else{
            neighbor.disorientation = std::numeric_limits<double>::max();
        }
    }
...
void PTMNeighborFinder::Query::calculateRMSDScale(){
    ...
    for(const Neighbor& neighbor : _list){
        rotatedTemplate.push_back(_orientation * neighbor.idealVector);
        centered.push_back(neighbor.delta);
        barycenter += neighbor.delta;
    }
